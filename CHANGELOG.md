...
- **The report's alignment panels no longer show sequence beside a hit unless `--padlen` is set.** Model positions a hit did not match are gaps by default; with `--padlen` they show the adjacent sequence, greyed. Those bases are not evidence for the model — they are whatever happens to lie next to the hit — and an alignment invites reading them as part of the match. The panel legend and caption follow the mode.
...
- `count_model_overlaps` counts overlapping hits between every pair of query models, run before cluster merging renames each hit to its cluster. Models named as direct left/right partners in a pairing map are excluded: the two termini of one element often share a short stretch of identity and hit each other's loci, `filter_best_model_per_locus` already assigns those loci to the better-scoring model, and counting them would bury the cross-family overlaps the heatmap exists to show.
...
- **`tirmite pair --report` writes a self-contained HTML report.** One file per run, with no external stylesheets, scripts, fonts or images, so it opens from disk and still works years later out of an archive. It contains:
  - Genome-browser annotation tracks for every sequence carrying a hit, with scroll-to-zoom, drag-to-pan and keyboard navigation. Each annotation shows strand as an arrow, pairing group as colour, and a thin line linking the two termini of an element. Clicking an annotation opens the same popup an element name in the tables opens; since a hit is drawn as little as two pixels wide at whole-contig zoom, a few pixels of slack around it count as a hit, and hovering uses the same test so a visible summary always marks a spot where a click will work. Clicking a paired terminus opens a popup carrying the element as a FASTA record — header line included, so what is displayed is exactly what the copy buttons and the download link produce — with copy-as-bases, copy-as-FASTA, a `<element_id>.fasta` download link, and reverse-complement, which flips the header too so a saved file cannot be mistaken for the plus-strand extraction.
  ...
  Two distinctions the report is careful about. A hit that only partially matched its model is drawn with a torn edge; one that is short because the contig ended gets a contig-end cap instead, because those are different findings. In the alignment panels the same split appears as blank (a model position the hit did not match) versus, under `--report-pad-model`, grey — the sequence that actually sits beside the hit, shown so you can see whether a truncated hit stopped at a real boundary or merely fell below the score threshold part-way through the model. Padding is opt-in because those bases are not evidence for the model, and an alignment invites reading them as part of the match; `--padlen` implies it, and a contig end still renders blank because that sequence does not exist. The report also reports model coverage and the span-based coverage `--mincov` filters on as separate, labelled quantities — they differ, and the latter can exceed 1.

  Report generation runs after every other output is on disk and its failures are logged rather than raised, so a broken visualisation cannot cost a completed analysis. Options: `--report-out`, `--report-title`, `--no-report-sequences`, `--report-max-seq-mb`, `--report-msa`, `--report-pad-model`, `--report-msa-max-rows`, `--report-max-hits`, `--report-max-rows`.
- **`tirmite search --report` writes a self-contained HTML report**, reusing the same annotation tracks, alignment panels and table machinery as the pairing report. It answers the questions a search run raises rather than a pairing run's:
  ...
  - **Cross-matches** — every locus where two groups both hit, as a group-by-group matrix and a table of individual loci.
  - **A second heatmap ordering the models by average-linkage clustering on the overlaps**, with the tree drawn above the columns. Groups there come from the hits alone, so a block that crosses a box in the first heatmap is a cluster map at odds with its own evidence. Distance is a Dice dissimilarity, so a model sharing most of its few hits ranks as close as one sharing many of many. Clustering is a small UPGMA implementation in `tirmite.report.cluster` rather than a SciPy dependency: the matrix is one row per query model, so the naive form is instant. Each leaf carries a coloured symbol naming the cluster that model was assigned to, keyed beside the heatmap, so a branch with mixed symbols is visibly a grouping the hits support but the cluster map splits; shape only changes once the eight palette hues are used up, so membership never rests on colour alone. Row labels carry each model's total hit count, which is what makes an off-diagonal count interpretable — five shared loci out of six hits is a different claim from five out of five hundred.
  - **A model-by-model heatmap of shared hit loci**, counted before clustering — the only point at which the model behind each hit is still known. Models are ordered by cluster then name, with a box drawn around each cluster's block, so colour inside a box is the expected agreement between a cluster's members and colour outside it is two unrelated models claiming the same sequence. The diagonal counts redundant hits from one model — two of its own hits at one locus — which is usually zero, since a model's genuine hits land at distinct loci. It is deliberately not the model's total hit count: totals run an order of magnitude above any overlap, so sharing a colour ramp with them would flatten the off-diagonal signal the figure exists to show. A query belonging to more than one cluster makes the layout ambiguous, so the report warns and states that it used the first cluster alphabetically. `check_cross_cluster_overlaps` logs only its first ten warnings so a bad cluster map cannot bury a run; it now also returns the full list, which is what the report shows.
