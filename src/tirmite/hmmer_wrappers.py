...
def cmdScriptHMMER(
    hmmDir: Optional[str] = None,
    hmmFile: Optional[str] = None,
    alnDir: Optional[str] = None,
    tempDir: Optional[str] = None,
    args=None,
) -> Tuple[List[str], str, str]:
    """
    DEPRECATED: Use process_hmmer_workflow() instead.
    ...
    hmmFile : str, optional
        Single HMM file path.
    alnDir : str, optional
        Directory containing alignment files.
    tempDir : str, optional
        Temporary directory path.
    args : object
        Arguments object with HMMER parameters.

    Returns
    -------
    tuple[list[str], str, str]
        - List of command strings (deprecated format)
        - Results directory path
        - HMM database directory path
    """
    import warnings

    warnings.warn(
        'cmdScriptHMMER() is deprecated. Use process_hmmer_workflow() instead.',
        DeprecationWarning,
        stacklevel=2,
    )

    # Convert args object to dictionaries
    executable_paths = {
        'hmmbuild': getattr(args, 'hmmbuild', 'hmmbuild'),
        'hmmpress': getattr(args, 'hmmpress', 'hmmpress'),
        'nhmmer': getattr(args, 'nhmmer', 'nhmmer'),
    }

    search_params = {
        'evalue': getattr(args, 'maxeval', None),
        'cores': getattr(args, 'cores', None),
        'nobias': getattr(args, 'nobias', False),
    ...
    try:
        ...
        # Return empty command list for compatibility
        return [], str(results_dir), str(hmm_db)

    except Exception as e:
        raise RuntimeError(f'HMMER workflow failed: {e}') from e
