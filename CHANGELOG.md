...
- `add_blast_header` streams the BLAST table through a temporary file instead of reading it into memory. Peak allocation was ~2x the file size (measured: 21 MB for a 10.6 MB table) and is now constant regardless of size. It also no longer truncates the original before rewriting, so an interrupted run can no longer destroy the search results.
- `tirmite validate` and `tirmite seed` build one BLAST database source per run instead of one per hit. `BlastDBSource` caches contig lengths because each miss costs a `blastdbcmd` subprocess, and rebuilding it discarded that cache every time: measured at 40 subprocess spawns for 20 hits on one contig, now 21. `tirmite seed` also indexed the genome once per target-ID check *and* again for extraction — three times over for an asymmetric run — and now indexes it once, lazily, so a run that fails before extraction never pays for it. `extract_hit_sequence` takes a source rather than a database path.
