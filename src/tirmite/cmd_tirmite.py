...
def mainArgs():
    ...
    parser.add_argument('--hmmDir', type=str,
                        default=None,
    ...
    parser.add_argument('--alnDir', type=str,
                        default=None,
    ...
    parser.add_argument('--maxeval', type=float,
                        default=0.001,
    ...
    parser.add_argument('--hmmpress', type=str,
                        default='hmmpress',
                        help='Set location of hmmpress if not in PATH.')
    parser.add_argument('--nhmmer', type=str,
                        default='nhmmer',
                        help='Set location of nhmmer if not in PATH.')
    parser.add_argument('--hmmbuild', type=str,
                        default='hmmbuild',
...
def main():
    ...
    if args.pairbed:
        ...
        hitTable = tirmite.import_BED(infile=args.pairbed,
                                      hitTable=hitTable,
                                      prefix=args.prefix)
        ...
        if args.nopairing:
            tirmite.writeTIRs(outDir=outDir,
                              hitTable=hitTable,
    ...
    # Else run nhmmer and load TIR hits.
        ...
        if args.alnDir or args.alnFile:
            stockholmDir = tirmite.convertAlign(alnDir=args.alnDir,
        ...
        cmds, resultDir, hmmDB = tirmite.cmdScript(hmmDir=args.hmmDir,
                                                   hmmFile=args.hmmFile,
                                                   alnDir=stockholmDir,
                                                   tempDir=tempDir,
                                                   args=args
                                                   )
        tirmite.run_cmd(cmds,
        ...
        for resultfile in glob.glob(os.path.join(os.path.abspath(resultDir), '*.tab')):
            ...
            hitTable = tirmite.import_nhmmer(infile=resultfile,
                                             hitTable=hitTable,
                                             prefix=args.prefix
                                             )
            modelCount += 1
    ...
    if args.reportTIR in ['all', 'paired']:
        ...
        tirmite.writePairedTIRs(outDir=outDir,
                                paired=paired,
                                hitIndex=hitIndex,
                                genome=genome,
                                prefix=args.prefix,
    ...
    pairedEles = tirmite.fetchElements(paired=paired,
                                       hitIndex=hitIndex,
                                       genome=genome)
    ...
    # Set gff file path
        ...
        tirmite.gffWrite(outpath=gffOutPath,
